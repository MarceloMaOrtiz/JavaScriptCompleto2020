<template>
    <!--
        Da mesma forma que no React não é possível utilizar elementos adjacents
        sendo necessário envolver por alguma outra tag os elementos

        Necessário utilizar os eventos da forma abaixo para possibilitar o envio de propriedades
        entre os módulos, ou seja, nesse caso estamos recebendo o 'label' do Button.vue
    -->
...
export default {
    // É utilizado uma função para data que irá retornar o estado inicial do objeto
    data: function() {
        return {
            displayValue: '0',
            clearDisplay: false,
    ...
    methods: {
        clearMemory() {
            // Object.assign(a, b) atribua 'b' a 'a'
            // this.$option.data() retornar o data desse component
        ...
        setOperation(operation) {
            if (this.current === 0) {
                ...
                try {
                    ...
                    console.log(e)
                    // Emit um erro que pode ser tratado, por exemplo, dentro do App.vue. Mas não está
                    // sendo tratado
                    this.$emit('onError', e)
                }
